...
function parsearMovimientosDeHtml(html, cid = null) {
  ...
  // Buscar número de expediente (CNT XXXXX/YYYY)
  const numMatch = html.match(/CNT\s+\d+\/\d+/);
...
async function obtenerTodosLosMovimientos(cookies, cid, htmlInicial) {
  const resultado = parsearMovimientosDeHtml(htmlInicial, cid);
...
async function buscarExpedientePorNumero(cookies, numeroExpediente) {
  ...
  // Parsear número (ej: "CNT 019429/2025" -> numero=019429, anio=2025)
  const match = numeroExpediente.match(/(\d+)\/(\d+)/);
  if (!match) {
  ...
  const expediente = resultado.expedientes.find(exp => 
...
async function scrapePjnMovimientosMasivo(usuario, password, expedientesSupabase) {
  ...
  // Crear mapa de números SCW -> expediente SCW
  const mapaScw = {};
  listaScw.expedientes.forEach((exp, idx) => {
    const numMatch = exp.numero.match(/(\d+)\/(\d{4})/);
    if (numMatch) {
      const clave = `${numMatch[1]}/${numMatch[2]}`;
      const clavePadded = `${numMatch[1].padStart(6, '0')}/${numMatch[2]}`;
      mapaScw[clave] = { ...exp, indexGlobal: idx };
      mapaScw[clavePadded] = { ...exp, indexGlobal: idx };
    }
  });
  ...
  for (const expSupa of expedientesSupabase) {
    ...
    const numMatch = num.match(/(\d+)\/(\d{4})/);
    if (numMatch) {
      const clave = `${numMatch[1]}/${numMatch[2]}`;
      const expScw = mapaScw[clave] || mapaScw[`${numMatch[1].padStart(6, '0')}/${numMatch[2]}`];
      if (expScw) {
  ...
  async function procesarChunk(chunk, workerId) {
    ...
    for (const item of chunk) {
      try {
        ...
        const busqueda = await buscarExpedientePorNumero(cookies, item.supabase.numero || item.supabase.numero_causa);
        ...
        console.log(`  [W${workerId}] ✅ ${item.scw.numero} - ${movs.length} movs`);
